...
Once you have the easiest problem you cannot solve, then it is time to
get to work!  So here's my proposal for an easier problem. Let's study
a subset of $\aut(T)$. Let's roll it way back to middle school. We'll
study the \textit{symmetries} of $T$, the symmetries of $T$ will be a
subset of $\aut(T)$. An informal definition of symmetry might be:
\begin{quote}
...
If we look up the word \textit{dihedral} in a dictionary, we get a
...
\end{quote}
Indeed, we'll be studying the symmetries of two-dimensional shapes.
...
\begin{example}[The dihedral group of order six]
  ...
  \begin{proof}
    ...
    $f\circ r$ as simply $fr$ and $r\circ r$ as $r^2$.
    \[
...
At this point we have made precise what ``symmetry'' means in terms of
geometric sets:
\begin{center}
  \textbf{Symmetries of the
$\boldsymbol{n}$-gon are exactly the isometries of an $\boldsymbol{n}$-gon.}
\end{center}

These symmetries (functions that are isometries on the $n$-gon that
map the $n$-gon to itself) will always form a group. This type of a
group is called a \textit{dihedral} group.

\begin{definition}
  A \dfn{dihedral group} is a group of symmetries of the regular
...
\begin{definition}
  ...
  \[
  \o(g) := \min\left\{n: \prodstar_{i=1}^n g = e\right\}\in\N.
  \]
...
We can also study the symmetries of objects in higher dimensions,
these are \textbf{not} dihedral groups, but they are still groups of
symmetries. Moving up in dimension, we'll find that working in $3$D
challenging enough. It's a fact, there are exactly
five regular solids, called \link[\textit{Platonic
    solids}]{https://en.wikipedia.org/wiki/Platonic\_solid}.
...
\begin{example}[The tetrahedral group]\label{EG:T12}
...
\begin{exercise}
  ...
  like the others do.
  \begin{hint}
    Investigate what a \link[dual polyhedron]{https://en.wikipedia.org/wiki/Dual_polyhedron} is.\index{dual polyhedron}
  \end{hint}
\end{exercise}
