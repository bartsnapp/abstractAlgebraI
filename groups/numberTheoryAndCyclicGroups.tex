...
\begin{corollary}[Unique factorization of natural numbers]\label{C:UFNN}
...
\begin{lemma}[Order divides other exponents]\label{L:odoe}
  Let $G$ be a group with identity $e$ and $a\in G$. If $a^n = e$, then $\o(a)|n$.
  \begin{sketch}
...
\begin{theorem}[Element's order divides cyclic group's order]\label{T:eodvgo}
  ...
  $\o(a)$ divides $|G|$.
  \begin{sketch}
    Now use the fact that $G$ is cyclic to show $a^{|G|} = e$.
  \end{sketch}
\end{theorem}
...
with the dihedral groups. So what is another connection to geometry?
