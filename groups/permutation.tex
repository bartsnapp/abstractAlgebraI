...
\begin{definition}\index{n-cycle@$n$-cycle}
  A \dfn{cycle} is a permutation $\sigma$ that can be represented as:
  \[
  ...
  sometimes called a \dfn{transposition}.
\end{definition}
...
\begin{definition}
  A \dfn{permutation matrix} is a matrix $P_\sigma$ whose entries are
...
\begin{exercise}
  Given a permuatation $\sigma = \left(\begin{smallmatrix}
    1 & 2 & 3 & \cdots & n\\
...
\begin{corollary}[Matrix decomposition]
...
\begin{example}[Alternating group on four symbols]
  ...
  group is known as the \dfn{alternating group on four symbols} and is
