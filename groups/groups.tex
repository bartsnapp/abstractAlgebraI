...
\begin{example}[The number $\boldsymbol{i}$]\label{E:C}
  ...
  \begin{proof}
    ...
    that we know that multiplication is associative. The identity
...
\begin{exercise}
  ...
  meaning they would write ``$(x)f$.'' Why would an author do this?
  \begin{hint}
    Rephrase the lemma above, writing the functions on the right, and
    see if you notice anything nice. In particular, think about this:
    \[
...
\begin{exercise}\label{E:SG}
  ...
  \end{align*}
  \end{quote}
  Explain what this ``groupie'' is talking about. Be very
...
\begin{definition}
  Let $(G,\star)$ be a group, a subset $\{x_1,\dots, x_n,\dots\}=X\subset
