...
\begin{example}[The number $\boldsymbol{i}$]\label{E:C}
...
\begin{example}[The group $\boldsymbol{C}$]\label{E:CC}
...
\begin{definition}\index{isomorphic!groups}\index{iso@$\iso$}\index{G=H@$G\iso H$}
...
\begin{definition}
  Let $(G,\star)$ be group, a subset $\{g_1,\dots, g_n,\dots\}\subset
  G$ is a set of \dfn{generators} for $G$ if for all $a\in G$, there
  exist $\alpha_i$ such that
  \[
  a =\left(\prodstar_{i=1}^m \prodstar_{k=1}^{\alpha_i}g_i \right) \star \left(\prodstar_{i=1}^m \prodstar_{k=1}^{\alpha_i}g^{-1}_i\right)
  \]
  where $m,\alpha_i\in \Z$ and the $g_i$'s are not necessarily distinct.

  
  If the group has an operation like multiplication, we write
  \begin{align*}
  a &= \left(\prod_{i=1}^m \prod_{k=1}^{\alpha_i}g_i\right)\cdot \left(\prod_{i=1}^m \prod_{k=1}^{\alpha_i}g_i^{-1}\right)\\
  &=  \prod_{i=-n}^m g_i^{\alpha_i}.
  \end{align*}


  If the group has an operation like addition, we write
  \begin{align*}
  a &= \left(\sum_{i=1}^m \sum_{k=1}^{\alpha_i}g_i\right)+\left(\sum_{i=1}^m \sum_{k=1}^{\alpha_i}-g_i\right)\\
  &=  \sum_{i=-n}^m \alpha_i \cdot g_i.
  \end{align*}
\end{definition}
...
\begin{itemize}
\item \link[\textit{The evolution of group theory: a brief survey}, I.\ Kleiner, Mathematics Magazine, Volume 59, 1986]{https://www.jstor.org/stable/2690312}.
\item \link[\textit{The group-theory element of the history of mathematics}, G.\ Miller, The Scientific Monthly, Volume 12, 1921]{https://www.jstor.org/stable/6754}.
\end{itemize}
