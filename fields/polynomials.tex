...
\begin{corollary}[$\boldsymbol{\pmb\Z_p^{\pmb\times}}$ is cyclic.]
...
\begin{theorem}[Fundamental theorem of algebra]\index{fundamental theorem of algebra}\label{T:fta}
  A polynomial in $\C[x]$ of degree $n$ will have exactly $n$ roots in
  $\C$.
  \begin{sketch}
    ...
    gave a proof that counted the intersections, and showed that if
    the polynomial has degree $n$, there will always be $n$
    intersections. For example, above we are working with a fifth
    degree polynomial, and we see we have $5$ intersections.
  \end{sketch}
\end{theorem}
