...
\begin{definition}
  ...
  of $m(x)$ and $n(x)$ as $\gcd(m(x),n(x))$. If $c\in K$ and $c\in
  \gcd(m(x),n(x))$ we say that $m(x)$ and $n(x)$ are \dfn{coprime} or
...
if $h(x)\in\gcd(m(x),n(x))$, $h(x)$ is simultaneously the polynomial of
\textbf{largest degree} that divides both $m(x)$ and $n(x)$ and the
polynomial of \textbf{smallest degree} of the form $f(x)\cdot m(x) + g(x)\cdot
...
\begin{lemma}[Arithmetic modulo a polynomial is well-defined]
...
\begin{theorem}[Congruence modulo an irreducible polynomial]\label{T:cmip}
