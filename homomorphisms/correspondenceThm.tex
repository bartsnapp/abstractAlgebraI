...
\begin{theorem}[Correspondence theorem]
  ...
  \begin{proof}
    ...
    \]
    We know that subgroups are preserved by $\pi_*$ via Lemma~\ref{L:imsubgp}.
    ...
    Since maps are bijective if and only if they have inverses, these
    maps are bijective.
    ...
    %% T = \pi_*(S) =\pi_*(S').
    %% \]
    %% In this case, $\pi^*(T) = S\cup S'$. Since neither $S$ nor $S'$
    %% can be a proper subgroup of the other (containment is preserved
    %% through the maps), it must be the case that $S=S'$, and hence
    %% $\pi_*$ is injective.
  \end{proof}
\end{theorem}
